...
4. 首次发布前，通过 Git Credential Manager 登录 GitHub；登录信息会安全保存在系统钥匙串中。
5. 在 Obsidian 设置 → 博客发布中确认项目、Node 和 Git 路径，之后直接点击“立即同步”。只有系统凭据不可用时，才需要临时填写仅授权该仓库、权限为 `Contents: Read and write` 的 fine-grained token。
